...
BUTTONS = [
    {"text": "🐶 UPUP  🐶", "url": "https://task.upupusa.com/#/pages/index/index?code=iob1y"},
    {"text": "📣 Join UPUP Community 📣", "url": "https://t.me/upup550"},
    {"text": "📱 Find Us on WhatsApp 📱", "url": "https://whatsapp.com/channel/0029Vb0Kb1r42DckfMHnBI29"}
]

async def set_menu_button(bot: Bot) -> None:
    """Set the Menu Button in the chat list as WebApp button"""
    await bot.set_chat_menu_button(
        menu_button=MenuButtonWebApp(text="💰Earn💰", web_app=WebAppInfo(url=WEBAPP_URL))
    )
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for button in BUTTONS:
        inline_keyboard.append([InlineKeyboardButton(button["text"], url=button["url"])])
...
async def open_webapp(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        "Click the button below to open UPUP app:",
        reply_markup=reply_markup
    )
